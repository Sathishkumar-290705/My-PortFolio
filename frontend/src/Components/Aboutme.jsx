...
const Aboutme = ({ data }) => {
  ...
  <h2 className='text-soft_coal mini:text-2xl nl:text-3xl mini:mt-12  mt:mt-12 np:mt-20 lp:mt-12  nl:mt-20'> About Me  </h2>
  ...
  <div className='flex flex-col nl:flex-row  w-[92%] mx-auto mt-8 gap-2 mini:px-2 px-12 '>
   ...
   <div className='  p-4 text-justify border-2 rounded-xl bg-muted_pale mt-4'>
    ...
    {userData[0]?.aboutMe?.content?.map((item,index)=>{
      return (
        <div key={index}>
              <p  className='mb-4 nl:mb-2 text-text mini:text-xs mt:text-sm nl:text-base text-base'> {item} </p>
              <br />
        </div>
      )
    })} 
   ...
   <div className='h-fit p-4 border-2 rounded-xl mt-2 bg-softened ' >
    <p className='text-text2 text-justify mini:text-xs mt:text-sm nl:text-base'> Lorem ipsum dolor sit amet consectetur adipisicing elit. Architecto dignissimos recusandae tempore odio totam! Accusamus illo fuga totam quos autem quae, corporis aut ea quibusdam. Ipsam necessitatibus earum soluta optio.</p>
   </div>
  </div>
