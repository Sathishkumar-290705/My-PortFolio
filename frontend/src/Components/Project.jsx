    ...
    <div className='mt-16 mini:w-[96%] np:w-[90%] flex flex-wrap gap-12 justify-center mx-auto' >
      {/* PROJECT LIST  */}
      {
        projectData && projectData?.map((project,index)=>{
          return(
       <div className='mt:w-[24rem]  min-h-80 max-h-88  max-w-[24rem] mini:w-[90%] np:w-[80%] mini:min-h-72 h-fit lp:w-[22rem] overflow-hidden border-2 border-text flex flex-col justify-between item-center bg-muted_pale p-4 '>
    
          <h4 className=' mini:text-xl text-text h-12 font-bold my-auto flex  items-center'> {project.title}</h4>
         <div className='flex flex-col justify-between flex-1 mt-4 '>
          <p className=' mini:text-xs  text-text min-h-20'>{project.description} </p>
